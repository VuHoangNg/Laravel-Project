...
const ScriptTab = ({ contentHeight, media1_id, scriptId, feedbackId }) => {
    ...
    if (feedbackId && fetchedFeedbackId.current !== feedbackId) {
      ...
      feedBackContext
        ...
        .then((feedbackData) => {
          ...
          if (!selectedScript) setSelectedScript({ key: parseInt(scriptId) });
          setShowFeedBackDrawer(true);
        })
        .catch((err) => {
  ...
  const handleEdit = (record) => {
    ...
    form.setFieldsValue({
      ...record,
      est_time: record.est_time ? dayjs(record.est_time, "HH:mm:ss") : null,
    });
  };
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      if (error.fieldErrors) {
        ...
        message.error(error.message || "Please fix the errors in the form.");
      } else {
        message.error(error.message || "An error occurred.");
      }
      // Modal stays open, form values are retained
    }
  };
  ...
  return (
    <div style={{ display: "flex", height: contentHeight, width: "100%" }}>
      ...
      >
        ...
        <Modal
          title={editingScriptContext.editingScript ? "Edit Script" : "Create Script"}
          ...
          width={600}
          style={{ top: 20 }}
          contentStyle={{ boxShadow: "none", background: "#FFFFFF", borderRadius: 8, padding: 24 }}
        >
          <Form
            form={form}
            onFinish={(values) => {
              const formattedValues = {
                ...values,
                est_time: values.est_time ? values.est_time.format("HH:mm:ss") : undefined,
              };
            ...
            initialValues={
              editingScriptContext.editingScript
                ? {
                    ...createScriptContext.formData,
                    est_time: createScriptContext.formData.est_time
                      ? dayjs(createScriptContext.formData.est_time, "HH:mm:ss")
                      : null,
                  }
                : createScriptContext.formData
            }
          ...
          >
            ...
            >
              <Input placeholder="Enter part name" size="large" style={{ width: "100%" }} />
            </Form.Item>
            <Form.Item
              name="est_time"
              label={
              ...
              rules={[
                { required: true, message: "Please select Est. Time" },
                {
                  validator: (_, value) =>
                    value && !dayjs(value).isValid()
                      ? Promise.reject(new Error("Invalid time format"))
                      : Promise.resolve(),
                },
              ]}
            >
              <TimePicker
                ...
                placeholder="Select time (e.g., 00:00:05)"
                size="large"
                style={{ width: "100%" }}
                showNow={false}
              />
            ...
            <Form.Item
              ...
              rules={[{ required: true, message: "Please enter Direction" }]}
            >
              <Input placeholder="Enter direction" size="large" style={{ width: "100%" }} />
            </Form.Item>
            <Form.Item
              name="detail"
              label={
        ...
        <Modal
          ...
          width={400}
          style={{ top: 20 }}
          contentStyle={{ boxShadow: "none", background: "#FFFFFF", borderRadius: 8, padding: 24, textAlign: "center" }}
        >
